...
public class BallMulti : MonoBehaviour {
    ...
    void Update() {
        ...
        poss1 = paddle.GetComponent<ShooterBehaviour2>().pos1;
        poss2 = paddle.GetComponent<ShooterBehaviour2>().pos2;
        poss3 = paddle2.GetComponent<ShooterBehaviour3>().pos1;
        poss4 = -2;
        ...
        objPos = Camera.main.ScreenToWorldPoint(objetoPausarV);
            ...
            if (Input.GetMouseButtonDown(0) && Time.timeScale == 1)
            {
                ...
                else if (transform.position.y >= paddle2.position.y && Disparo.active)
                {
                    Disparar.Play();
                    rbBall.constraints = RigidbodyConstraints2D.None;
                    rbBall.constraints = RigidbodyConstraints2D.FreezeRotation;
